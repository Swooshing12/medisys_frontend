...
class MisCitasMedicoSystem {
    ...
    init() {
        console.log('👨‍⚕️ Iniciando sistema de mis citas médicas...');
        
        this.setupEventListeners();
        this.loadInitialData();
        
        console.log('✅ Sistema de mis citas listo');
    }
    ...
    cargarCitasHoy() {
        ...
        this.showSuccess('Mostrando citas de hoy: ' + today);
    }
    
    async loadInitialData() {
        console.log('📊 Cargando mis citas iniciales...');
        await this.buscarMisCitas();
    }
    ...
    async buscarMisCitas() {
        ...
        try {
            ...
            console.log('👨‍⚕️ Buscando mis citas con filtros:', filters);
            
            const queryParams = new URLSearchParams(filters).toString();
            const response = await fetch(`/dashboard/api/mis-citas/?${queryParams}`);
        ...
        row.innerHTML = `
            ...
            <td>
                <div class="paciente-info">
                    ...
                    ${cita.paciente.edad ? `<small class="text-muted">${cita.paciente.edad} años</small>` : ''}
                </div>
            </td>
            <td>
                <div class="contacto-info">
                    ...
                    ` : ''}
                    ${cita.paciente.tipo_sangre ? `
                        <div class="tipo-sangre">
                            <span class="badge bg-danger">${cita.paciente.tipo_sangre}</span>
                        </div>
                    ` : ''}
                </div>
            </td>
            <td>
                ...
                ${cita.paciente.alergias ? `
                    <div class="alert alert-warning alert-sm mt-1 p-1">
                        <small><i class="fas fa-exclamation-triangle"></i> ${this.truncateText(cita.paciente.alergias, 30)}</small>
                    </div>
                ` : ''}
            </td>
            <td>
            ...
            <td>
                <div class="action-buttons">
                    ...
                    ` : ''}
                    ${!cita.tiene_consulta && (cita.estado === 'Confirmada' || cita.estado === 'Pendiente') ? `
                        <button class="btn btn-sm btn-outline-info" onclick="iniciarConsulta(${cita.id_cita})" title="Iniciar consulta">
    ...
    updatePagination(paginacion) {
        ...
        paginationHTML += `
            <li class="page-item ${!paginacion.tiene_anterior ? 'disabled' : ''}">
                <a class="page-link" href="#" onclick="misCitasMedicoSystem.goToPage(${paginacion.anterior_pagina || 1})">
        ...
        for (let i = startPage; i <= endPage; i++) {
            paginationHTML += `
                <li class="page-item ${i === paginacion.pagina_actual ? 'active' : ''}">
                    <a class="page-link" href="#" onclick="misCitasMedicoSystem.goToPage(${i})">${i}</a>
                </li>
            `;
        }
        ...
        paginationHTML += `
            <li class="page-item ${!paginacion.tiene_siguiente ? 'disabled' : ''}">
                <a class="page-link" href="#" onclick="misCitasMedicoSystem.goToPage(${paginacion.siguiente_pagina || paginacion.total_paginas})">
...
// Funciones globales para los botones de acción (reutilizar del consulta-citas.js)
...
function iniciarConsulta(idCita) {
    console.log('▶️ Iniciar consulta médica:', idCita);
    Swal.fire({
        icon: 'info',
        title: 'Función en Desarrollo',
        text: 'La función de iniciar consulta estará disponible pronto',
        confirmButtonColor: '#007bff'
    });
}
...
// Reutilizar la función mostrarModalDetalle del consulta-citas.js
// (Puedes copiarla completa desde el otro archivo)

// Initialize cuando el DOM esté listo
document.addEventListener('DOMContentLoaded', function() {
    console.log('🚀 Iniciando sistema de mis citas médicas...');
    window.misCitasMedicoSystem = new MisCitasMedicoSystem();
});